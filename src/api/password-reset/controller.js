...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Foodtalk Backend account.<br>
        ...
        &mdash; Foodtalk Backend Team
      `
      return sendMail({ toEmail: email, subject: 'Foodtalk Backend - Password Reset', content })
    })
